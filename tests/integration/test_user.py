...
async def get_all_usernames(httpx_client: AsyncClient) -> list[str]:
...
async def three_groups(
...
) -> tuple[group_module.LocalGroup, LocalGroupData, group_module.LDAPGroup]:
    ...
    assert isinstance(local_group_saved, group_module.LocalGroup)
    ...
    assert isinstance(ldap_group_saved, group_module.LDAPGroup)
    assert ldap_group_saved._data["type"] == SourceType.LDAP.value

    local_group_unsaved = group_module.new(display_name="Another test local group")
    assert isinstance(local_group_unsaved, LocalGroupData)
...
async def ldap_user(
    ...
    assert isinstance(ldap_user, user_module.LDAPUser)
...
def _test_fields_contract() -> None:
    ...
    expected_fields = {"username", "password", "is_super_user", "first_name", "last_name", "email", "groups"}
...
async def _test_local_user_data_api(
    ...
    local_group, local_group_data, ldap_group = three_groups
    ...
    user.groups = [local_group.id, ldap_group.id]
    ...
    with pytest.raises(ConflictError, match=r"You cannot add user to LDAP group"):
        await adcm_client.users.init(user)

    user.groups = [local_group.id]
    local_user = await adcm_client.users.init(user)

    assert isinstance(local_user, user_module.LocalUser)
    ...
    groups = await local_user.groups
    assert isinstance(groups, list)
    assert len(groups) == 1
    assert isinstance(groups[0], group_module.LocalGroup)
    assert groups[0].id == local_group.id
    ...
    assert isinstance(edited_user, user_module.LocalUserLazy)
    with pytest.raises(AttributeError):
        await edited_user.delete()

    saved_user = await edited_user.save()
    assert isinstance(saved_user, user_module.LocalUser)
...
async def _test_local_user_lazy_api(
    adcm_client: ADCMClient,
    httpx_client: AsyncClient,
    three_groups: tuple[group_module.LocalGroup, LocalGroupData, group_module.LDAPGroup],
) -> None:
    ...
    assert isinstance(user, user_module.LocalUserLazy)
    assert username not in await get_all_usernames(httpx_client)
    ...
    groups = await saved_user.groups
    assert isinstance(groups, list)
    assert len(groups) == 1
    assert isinstance(groups[0], group_module.LocalGroup)
    assert groups[0].id == local_group.id
...
async def _test_ldap_user_api(user: user_module.LDAPUser) -> None:
    assert isinstance(user, user_module.LDAPUser)
...
async def _test_users_accessor(
    ...
    group = await adcm_client.groups.init(local_group_unsaved)  # create group
    assert isinstance(group, group_module.LocalGroup)
    assert isinstance(group.id, int)
    groups: dict[str, int] = {group.display_name: group.id for group in (local_group_saved, group)}
    ...
    # get
    assert isinstance(await adcm_client.users.get(username__eq="User_30"), user_module.LocalUser)
    ...
    # get_or_none
    assert isinstance(await adcm_client.users.get_or_none(username__eq="User_2"), user_module.LocalUser)
    ...
    assert all(isinstance(user, user_module.LocalUser | user_module.LDAPUser) for user in all_users)
